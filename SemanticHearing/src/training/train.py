...
def train_epoch(
    ...
    tensorboard_trace_handler = torch.profiler.tensorboard_trace_handler(
        writer.log_dir
    )  # analyze theoperation in the model, leave it here
        ...
        for batch_idx, (inp, tgt) in enumerate(train_loader):
            ...
            if writer is not None and batch_idx == 0:
                train_loader.dataset.tensorboard_add_sample(
                    writer, tag="Train", sample=(inp, output, tgt), step=epoch
                )
...
def train(args: argparse.Namespace):
    ...
    logging.info("Loaded train dataset containing %d elements" % (len(data_train)))
    ...
    try:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            curr_train_metrics = train_epoch(
                model,
                device,
                optimizer,
                train_loader,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_test_items", type=int, default=None, help="Number of items to test."
    )
    parser.add_argument("--start_epoch", type=int, default=0, help="Start epoch")
    parser.add_argument("--pretrain_path", type=str, help="Path to pretrained weights")
    parser.add_argument(
        "--use_cuda", dest="use_cuda", action="store_true", help="Whether to use cuda"
    )
    parser.add_argument(
        "--gpu_ids",
        nargs="+",
